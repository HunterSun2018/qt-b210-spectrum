    ...
    DemodulatorState *state)
{
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        while (state->resampleCursor <= state->sampleIndex) {
            ...
            const float left = state->lastResampleSample.left
                + frac * (current.left - state->lastResampleSample.left);
            const float right = state->lastResampleSample.right
                + frac * (current.right - state->lastResampleSample.right);
